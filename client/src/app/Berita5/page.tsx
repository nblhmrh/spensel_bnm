  ...
  useEffect(() => {
    const fetchBerita = async () => {
      try {
        ...
        const berita5Data = response.data.find((item: Berita) =>
          item.slug && item.slug.toLowerCase().includes('berita5')
        );
        setBerita(berita5Data || null);
      } catch (error) {
      ...
      <div className="bg-[#154472] w-full h-[300px]">
        ...
        <section className="flex flex-col py-8 px-6">
          <h1 className="text-3xl md:text-4xl font-bold text-[#ffff]">
            {loading ? 'Memuat...' : berita?.judul || 'Berita 5'}
          </h1>
          <p className="text-white mt-2 py-3 font-normal">
            ...
            &gt;{" "}
            <Link href="/berita5" className="hover:text-gray-300">
              Berita 5
            </Link>
          </p>
        </section>
