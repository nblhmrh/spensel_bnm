  ...
  useEffect(() => {
    const fetchBerita = async () => {
      try {
        ...
        // Cari berita dengan slug yang mengandung 'berita3' (case-insensitive)
        const berita3Data = response.data.find((item: Berita) =>
          item.slug && item.slug.toLowerCase().includes('berita3')
        );
        setBerita(berita3Data || null);
      } catch (error) {
      ...
      <div className="bg-[#154472] w-full h-[300px]">
        ...
        <section className="flex flex-col py-8 px-6">
          <h1 className="text-3xl md:text-4xl font-bold text-[#ffff]">
            {loading ? 'Memuat...' : berita?.judul || 'Berita 3'}
          </h1>
          <p className="text-white mt-2 py-3 font-normal">
            ...
            &gt;{" "}
            <Link href="/berita3" className="hover:text-gray-300">
              Berita 3
            </Link>
          </p>
        </section>
